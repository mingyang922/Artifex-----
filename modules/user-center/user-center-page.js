...
const userCenterAvatar = window.UserCenterAvatar;
const userCenterSession = window.UserCenterSession;
...
function saveUserConfig() {
    ...
    const config = {
        ...
        // 头像历史记录
        avatarHistory: userCenterAvatar.getAvatarHistoryFromStorage(),
    };
...
function loadUserConfig() {
    ...
    if (config.currentAvatar) {
        userCenterAvatar.setCurrentAvatar(config.currentAvatar);
    }

    // 加载并渲染头像历史记录
    const avatarHistory = config.avatarHistory || [];
    userCenterAvatar.renderAvatarHistory(avatarHistory);
}
        ...
        if (e.target.files && e.target.files[0]) {
            ...
            reader.onload = function (e) {
                ...
                img.onload = function () {
                    ...
                    // 设置为当前头像
                    userCenterAvatar.setCurrentAvatar(compressedImageData);

                    // 添加到头像历史记录
                    userCenterAvatar.addToAvatarHistory(compressedImageData);
...
window.addEventListener('DOMContentLoaded', async () => {
    const ok = await userCenterSession.syncSessionAndProfile();
    if (!ok) return;
    ...
    // 初始化删除头像功能
    userCenterAvatar.initDeleteAvatarFunctionality();

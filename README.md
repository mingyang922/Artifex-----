...
登录后可从侧边栏进入“生产工作台”，统一管理可复现生成历史、角色一致性档案、项目成员与邀请、只读交付分享、批注审核、版本恢复、LoRA 训练、实时通知和成本预警。“研究与模板”页提供 7 类游戏美术工作流模板、可解释角色约束评估、真实生产指标、墨影示例项目以及服务端工作区备份。

其中“角色一致性档案”是基于 Seed、提示词、锁定属性和参考图的简化工作流，不等同于 embedding 或多图一致性模型；LoRA 任务只有在配置外部训练 Worker 后才会实际执行。完整完成度与功能边界见 `docs/CAPABILITY_MATRIX.md`。

...
interface HunyuanRequestData {
    [key: string]: unknown;
}

interface TencentStatusResponse {
    configured: boolean;
    hasOwnKeys: boolean;
    usesPlatformKeys: boolean;
    isAdmin: boolean;
    validation: CredentialValidation;
    secretIdPrefix: string | null;
    hint: string;
    liveTest?: string;
    message?: string;
    liveError?: string;
...
function loadTencentCloudSdk(): any {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    return require('tencentcloud-sdk-nodejs');
}

// 腾讯云混元API代理（用户级配置，使用官方SDK）
async function handleHunyuanProxy(
    req: Request,
    res: Response,
    { runtimeConfig, API_CONFIG, getUserProviderConfig, isAdminUser }: ProviderDeps,
): Promise<void> {
    try {
        const userTencentConfig: ApiCredentials | null = getUserProviderConfig(req.currentUser!.id, 'tencent');
        ...
        const isAdmin: boolean = isAdminUser(req.currentUser!);
        const hasOwnKeys: boolean = !!(userTencentConfig && userTencentConfig.secretId && userTencentConfig.secretKey);
        ...
        const camCheck: CredentialValidation = validateTencentCamCredential(SecretId, SecretKey);
        if (!camCheck.ok) {
            res.status(400).json({
                error: 'API密钥无效',
        ...
        const tencentcloud = loadTencentCloudSdk();
        ...
        const result = await client.ChatCompletions(requestData as HunyuanRequestData);
...
// 腾讯云混元图生图（线稿/草图→成品图）
async function callTencentImage2ImageAPI(
    prompt: string,
    imageDataUrl: string,
    ...
    const numStrength: number = typeof strength === 'number' ? strength : parseFloat(strength as string) || 0.7;
    const params = {
    ...
    const response: any = await client.ImageToImage(params);
    if (response.ResultImage) {
        return response.ResultImage as string;
    }
    throw new Error('腾讯云图生图返回格式错误，未找到图片数据');
}

// 腾讯云混元生图API调用
async function callTencentImageAPI(
    prompt: string,
    size: string,
    userConfig: ApiCredentials | null,
): Promise<string> {
    ...
    try {
        ...
        const response: any = await client.TextToImageRapid(params);
...
// 腾讯云密钥与连通性自检
async function handleTencentStatus(
    req: Request,
    res: Response,
    { getUserProviderConfig, isAdminUser }: StatusDeps,
): Promise<void> {
    const userCfg: ApiCredentials | null = getUserProviderConfig(req.currentUser!.id, 'tencent');
    const hasOwnKeys: boolean = !!(userCfg && userCfg.secretId && userCfg.secretKey);
    const { secretId, secretKey } = getTencentCamCredentials(userCfg);
    const validation: CredentialValidation = validateTencentCamCredential(secretId, secretKey);
    const isAdmin: boolean = isAdminUser ? isAdminUser(req.currentUser!) : false;
    const out: TencentStatusResponse = {

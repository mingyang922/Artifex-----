...
import dns from 'dns';
import { generateMockImage, callFreeImageAPI } from '../lib/utils';
import { imageGenerationLimiter } from '../lib/rate-limiter';
...
import type { User, ApiCredentials, QuotaCheck } from '../lib/types';

const dnsPromises = dns.promises;

// ── 依赖接口 ──────────────────────────────────────────────

interface ImageRouterDeps {
    ...
    logApiCall: (userId: number, provider: string, operation: string, status: string, duration: number) => void;
    checkQuota: (userId: number, provider: string) => QuotaCheck;
}

// ── 图片生成调度上下文接口 ────────────────────────────────

interface ImageDispatchContext {
    provider: string;
    prompt: string;
    ...
    getUserProviderConfig: (userId: number, provider: string) => ApiCredentials | null;
    req: Request;
    setOnJimengFrameSuccess: (fn: (url: string) => void) => void;
}

// ── 路由创建函数 ──────────────────────────────────────────

function createImageRouter(deps: ImageRouterDeps): Router {
    const router = Router();
    const { requireAuth, csrfProtection, isAdminUser, getUserProviderConfig, logApiCall, checkQuota } = deps;

    // ── 文生图 / 图生图 ──────────────────────────────────────────
    router.post('/image-proxy', requireAuth, csrfProtection, imageGenerationLimiter.middleware(), async (req: Request, res: Response): Promise<void> => {
        try {
            const {
                prompt,
                size,
            ...
            const normalizedProvider: string = String(provider || '').trim().toLowerCase();
            const currentIsAdmin: boolean = isAdminUser(req.currentUser);

            // 即梦尺寸校验
            if (normalizedProvider === 'jimeng') {
                const sizeCheck = jimengProvider.validateJimengSize(size);
                if (!sizeCheck.ok) {
                    res.status(400).json({ error: '参数错误', message: sizeCheck.message, provider: 'jimeng' });
                    return;
                }
            }

            if (!prompt) {
                res.status(400).json({ error: '参数错误', message: 'prompt参数不能为空' });
                return;
            }

            // 检查配额
            if (typeof checkQuota === 'function') {
                const quota: QuotaCheck = checkQuota(req.currentUser!.id, normalizedProvider);
                if (!quota.ok) {
                    res.status(429).json({
                        error: '调用次数超限',
            ...
            if (normalizedProvider === 'tencent') {
                ...
                const hasOwnKeys: boolean = !!(userTencentCfg && userTencentCfg.secretId && userTencentCfg.secretKey);
            ...
            const isImg2Img: boolean = mode === 'img2img';
            let effectivePrompt: string = prompt;
            let effectiveImageDataUrl: string | undefined = imageDataUrl;
            let effectiveStrength: number = strength;
            let onJimengFrameSuccess: ((url: string) => void) | null = null;
            ...
            if (normalizedProvider === 'jimeng' && !isImg2Img && styleReferenceImage && typeof styleReferenceImage === 'string') {
                const j: Record<string, any> = req.body.jimeng && typeof req.body.jimeng === 'object' ? { ...req.body.jimeng } : {};
                const eb: Record<string, any> = j.extra_body && typeof j.extra_body === 'object' ? { ...j.extra_body } : {};
            ...
            try {
                const imageUrl: string = await dispatchImageGeneration({
                    provider: normalizedProvider,
                    ...
                    req,
                    setOnJimengFrameSuccess: (fn: (url: string) => void) => { onJimengFrameSuccess = fn; },
                });

                // 即梦图生图回调
                if (normalizedProvider === 'jimeng' && isImg2Img && typeof onJimengFrameSuccess === 'function') {
                    (onJimengFrameSuccess as (url: string) => void)(imageUrl);
                }
            ...
            const provider: string = String(req.body?.provider || 'unknown').toLowerCase();
            const mode: string = req.body?.mode === 'img2img' ? '(图生图)' : '';
    ...
    // ── 图片代理（拉取远程图片） ─────────────────────────────────
    router.get('/proxy-image', requireAuth, async (req: Request, res: Response): Promise<void> => {
        ...
        try {
            ...
            if (!['http:', 'https:'].includes(parsed.protocol)) {
                res.status(400).json({ error: '只允许 http/https 协议' });
                return;
            }
            ...
            try {
                const { address: resolved } = await dnsPromises.lookup(hostname, { family: 0 });
                if (resolved && blockedPatterns.some(pattern => pattern.test(resolved))) {
                    res.status(403).json({ error: '禁止访问内网地址（DNS 解析）' });
                    return;
                }
            } catch (_) {
                // DNS 解析失败时阻止请求（防止绕过 SSRF 防护）
                res.status(502).json({ error: 'DNS 解析失败，无法验证目标地址安全性' });
                return;
            }
        } catch (e) {
        ...
        // 限制允许的 Content-Type
        const ALLOWED_CONTENT_TYPES: string[] = ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/svg+xml', 'image/bmp', 'image/tiff'];
        ...
        try {
            ...
            const contentType: string = String(response.headers['content-type'] || 'image/png').split(';')[0].trim().toLowerCase();
            // 只允许图片类型，防止代理 HTML/JS 等内容（XSS 风险）
            if (!ALLOWED_CONTENT_TYPES.some(ct => contentType.startsWith(ct))) {
                res.status(403).json({ error: '不允许的内容类型', contentType });
                return;
            }
...
async function dispatchImageGeneration(ctx: ImageDispatchContext): Promise<string> {
    const {

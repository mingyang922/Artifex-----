...
import type { TencentCamCredentials, CredentialValidation, ApiCredentials } from './types';

// —— 腾讯云 CAM 密钥工具 ——

interface TencentValidationResult {
    ok: boolean;
    code?: string;
    message?: string;
}

interface ErrorWithTencentCode extends Error {
    tencentValidationCode?: string;
}
...
function getTencentCamCredentials(userConfig?: UserConfig | null): TencentCamCredentials {
    const cfg: UserConfig = userConfig && typeof userConfig === 'object' ? userConfig : {};
    return {
...
function validateTencentCamCredential(secretId: string, secretKey: string): CredentialValidation {
...
function assertTencentCamCredential(secretId: string, secretKey: string): void {
    const v = validateTencentCamCredential(secretId, secretKey);
    if (!v.ok) {
        const err: ErrorWithTencentCode = new Error(v.message);
...
function generateSignature(
    ...
    endpoint: string = 'hunyuan.tencentcloudapi.com'
): string {
    const method = 'POST';
    const canonicalUri = '/';
...
interface JimengImageResponse {
    task_status?: string;
    error?: { message?: string; message_cn?: string };
    message?: string;
    data?: Array<string | { url?: string; b64_json?: string; size?: string }> | { url?: string; size?: string };
    output?: {
        images?: Array<{ url?: string; base64?: string; size?: string }>;
        text?: string;
    };
...
function normalizeJimengApiKey(raw: string | null | undefined): string {
...
function normalizePixelSize(sizeStr: string | null | undefined): string | null {
 ...
 */
function parseJimengImageResponse(data: JimengImageResponse | null | undefined): string | null {
    if (!data) return null;
    ...
    if (Array.isArray(data.data) && data.data.length) {
        ...
        if (first && typeof first === 'object' && first.b64_json) return `data:image/png;base64,${first.b64_json}`;
    }
    if (data.data && !Array.isArray(data.data) && typeof data.data.url === 'string') return data.data.url;
...
function extractJimengOutputSize(data: JimengImageResponse | null | undefined): string | null {
    if (!data || typeof data !== 'object') return null;

    const fromDataArr = Array.isArray(data.data) && data.data[0] && typeof data.data[0] === 'object' && data.data[0].size;
    if (fromDataArr) return normalizePixelSize(String(fromDataArr));
    if (data.data && !Array.isArray(data.data) && data.data.size) return normalizePixelSize(data.data.size);
...
function clamp01Range(v: unknown, min: number, max: number, fallback: number): number {
    const n = Number(v);
...
async function callFreeImageAPI(prompt: string, size?: string): Promise<string> {
...
<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">
  ...
  <text x="50%" y="57%" text-anchor="middle" fill="#dbeafe" font-size="${Math.max(14, Math.round(w / 48))}" font-family="Segoe UI, Arial">${safePrompt}</text>
        ...
        throw new Error(`免费API调用失败: ${msg}`);
    }
}

// —— DashScope 多模态解析 ——

function extractDashscopeMultimodalText(data: DashscopeResponse | null | undefined): string | null {
    if (!data || typeof data !== 'object') return null;
 ...
 * @param text - 要加密的文本
 * @returns 加密后的文本（IV:AuthTag:密文 格式）
 */
...
function decryptText(encryptedText: string): string {
    ...
    try {
        ...
        console.warn('[utils] decryptText 解密失败，可能密钥已变更:', msg);
        return '';
    }
}
 ...
 * 加密 JSON 对象中的敏感字段
 * @param obj - 要加密的对象
 * @param fields - 要加密的字段名
 * @returns 加密后的对象
 */
function encryptSensitiveFields(obj: Record<string, unknown>, fields: string[] = ['apiKey', 'secretKey', 'secretId']): Record<string, unknown> {
    if (!obj || typeof obj !== 'object') return obj;
    const result = { ...obj };
    for (const field of fields) {
        if (result[field] && typeof result[field] === 'string') {
            result[field] = encryptText(result[field] as string);
        }
    }
    return result;
}
 ...
 * 解密 JSON 对象中的敏感字段
 * @param obj - 要解密的对象
 * @param fields - 要解密的字段名
 * @returns 解密后的对象
 */
function decryptSensitiveFields(obj: Record<string, unknown>, fields: string[] = ['apiKey', 'secretKey', 'secretId']): Record<string, unknown> {
    if (!obj || typeof obj !== 'object') return obj;
    const result = { ...obj };
    for (const field of fields) {
        if (result[field] && typeof result[field] === 'string') {
            result[field] = decryptText(result[field] as string);
        }
    }
    return result;
}

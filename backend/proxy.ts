...
import { createImageRouter } from './routes/image-proxy';
import { createProjectRouter } from './routes/projects';
import { createAssetLibraryRouter } from './routes/asset-library';
import { createAdminRouter } from './routes/admin';
import { createAiProviderRouter } from './routes/ai-providers';
const usersDb = require('./db/users-db');
import { createAuthPolicy } from './lib/auth-policy';
import { createUserApiSettingsHelpers } from './lib/user-api-settings';
...
envFiles.forEach((envPath: string) => dotenv.config({ path: envPath, override: false }));
...
const resolvedSessionSecret: string | null = resolveSessionSecret();
...
app.use(compression({
    filter: (req: Request, res: Response) => {
        ...
        if (/image\/(jpeg|png|webp|gif)/.test(type as string)) return false;
...
// ── 代理检测与修复 ────────────────────────────────────────────────
function normalizeNoProxy(existing: string | undefined, hosts: string[]): string {
...
function disableDeadLocalProxyIfPresent(): void {
...
// ── CORS ──────────────────────────────────────────────────────────
const corsAllowedList: string[] = process.env.ALLOWED_ORIGINS
...
// ── CSRF ──────────────────────────────────────────────────────────
function generateCsrfToken(): string { return crypto.randomBytes(32).toString('hex'); }
...
function csrfProtection(req: Request, res: Response, next: NextFunction): void {
    ...
    const headerVal = (req.headers['x-xsrf-token'] as string) || (req.body && req.body._csrf) || '';
    if (!cookieVal || !headerVal || cookieVal !== headerVal) {
        res.status(403).json({ error: 'CSRF 令牌无效，请刷新页面重试' });
        return;
    }
    next();
}
...
app.get('/', (req: Request, res: Response) => res.redirect('/login.html'));
...
// 当前用户的活动日志
app.get('/api/activity-log', requireAuth, (req: Request, res: Response) => {
...
// 管理员：所有用户的活动日志
app.get('/api/admin/activity-log', requireAuth, (req: Request, res: Response) => {
...
for (const action of ['update', 'validate', 'reset']) {
    app.all(`/api/config/${action}`, requireAuth, (req: Request, res: Response) => {
...
// 详细健康检查（需要鉴权，暴露配置状态）
app.get('/api/health/detail', requireAuth, (req: Request, res: Response) => {
    if (!isAdminUser(req.currentUser)) {
        return res.status(403).json({ error: '权限不足' });
    }
...
// ── 启动服务器 ───────────────────────────────────────────────────
const PORT: number = Number(process.env.PORT) || API_CONFIG.proxy.port;
